...
- (void)testFindUnauthorizedByClassAndId
{
  var delegateMock = moq();
  var delegateWasCalled = NO;
  [delegateMock selector:@selector(crysonSession:failedToAuthorizeByClass:andId:) callback:function(args) {
      [OJAssert assert:TestEntity equals:args[1]];
      [OJAssert assert:1 equals:args[2]];
      delegateWasCalled = YES;
    }];

  var session = [self givenCrysonSessionWithRemoteEntity:{"id":1, "crysonEntityClass":"TestEntity", "crysonUnauthorized":true} andDelegate:delegateMock];
  [session findByClass:TestEntity andId:1 fetch:nil];
...
- (void)testFindAllByClassWithUnauthorizedEntity
{
  var delegateMock = moq();
  var delegateWasCalled = NO;
  [delegateMock selector:@selector(crysonSession:foundAll:byClass:) callback:function(args) {
  ...
  var session = [self givenCrysonSessionWithRemoteEntities:[{"id":1, "name":"test", "crysonEntityClass":"TestEntity"},
                                                            {"id":2, "crysonEntityClass":"TestEntity", "crysonUnauthorized":true}]
...
{
  return [self givenCrysonSessionWithRemoteEntities:[jsObject] andDelegate:aDelegate];
}

- (CrysonSession)givenCrysonSessionWithRemoteEntities:(CPArray)jsObjects andDelegate:(id)aDelegate
{
  var remoteService = moq();
  var getCallback = function(args /*requestUrl, delegate, onSuccess, onError, context*/) {
    if (args[2] == @selector(findByClassAndIdSucceeded:context:)) {
      [args[1] performSelector:args[2] withObject:jsObjects[0] withObject:args[4]];
    } else if (args[2] == @selector(findAllByClassSucceeded:context:)) {
      [args[1] performSelector:args[2] withObject:jsObjects withObject:args[4]];
    }
  };
